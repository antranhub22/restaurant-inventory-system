...
const inventory = sampleData.sample_inventory_current;
const items = sampleData.sample_items;

function getItem(item_id: number) {
  return items.find((i: any) => i.id === item_id);
}
          ...
          return (
            <Card key={inv.item_id} header={item?.name || 'Không rõ'}>
              <div className="flex items-center gap-2 mb-2">
                ...
                {inv.current_stock <= item?.min_stock && <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded">Dưới mức tối thiểu</span>}
              </div>
              <div className="mb-1">Tồn kho hiện tại: <span className="font-semibold">{inv.current_stock} {item?.unit_id ? item.unit_id : ''}</span></div>
